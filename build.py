...
Fetches all content from WP REST API and generates a complete static site.
Run locally or as a Vercel build step.
...
def fetch_all_posts():
    """Fetch every post from the WP REST API (handles pagination)."""
    all_posts = []
    page = 1
    while True:
        url = f"{WP_API}/posts?per_page=100&page={page}&_fields=id,slug,title,date,link,excerpt,content,categories"
...
.bcard{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:32px;transition:transform var(--t),border-color var(--t);display:flex;flex-direction:column}
...
def header(title, desc="", canonical="/", extra=""):
    if not desc:
...
def page_home():
    schema = json.dumps({
        ...
        "name":"Dr. Connor Robertson","url":SITE_URL,
    ...
    return header("Dr. Connor Robertson | Pittsburgh Entrepreneur & Business Strategist",
        "Dr. Connor Robertson is a Pittsburgh-based entrepreneur, educator, and philanthropist helping entrepreneurs scale businesses, build legacies, and create lasting impact.","/",extra) + f"""
...
<div class="feat-logos"><span>CXO Dispatch</span><span>C-Suite Brief</span><span>NY Wire</span></div>
</div></section>
<section class="sec"><div class="ctn">
<h2 class="sec-t">Dr. Connor Robertson is a Canadian-born entrepreneur, real estate innovator, educator, and philanthropist, and speaker.</h2>
...
def page_about():
    return header("About Dr. Connor Robertson | Entrepreneur, Strategist & Media Founder",
        "Dr. Connor Robertson is a visionary entrepreneur and educator focused on innovation and impact.","/about/") + """
...
def page_books():
    ...
    return header("Books by Dr. Connor Robertson | Business & Entrepreneurship",
        "Explore books by Dr. Connor Robertson on wealth building, business acquisitions, and entrepreneurship.","/books/") + f"""
<section class="pg-hero"><div class="ctn">
...
<section class="sec"><div class="ctn">
<div class="feat" style="padding:48px 0;margin-bottom:48px;border-bottom:none"><div class="feat-logos"><span>CXO Dispatch</span><span>C-Suite Brief</span><span>NY Wire</span></div></div>
...
def page_blog_index(posts, page_num, total_pages):
    ...
    cards = ""
    for p in batch:
        exc = re.sub(r'<[^>]+>', '', p.get("excerpt","")).strip()
        if len(exc) > 200: exc = exc[:200] + "..."
        try: dt = datetime.fromisoformat(p["date"]).strftime("%B %d, %Y")
        except: dt = ""
        cards += f'<a href="{p["relative_url"]}" class="bcard"><h3>{p["title"]}</h3><p class="exc">{esc(exc)}</p><span class="meta">{dt}</span></a>\n'
    pag = '<div class="pag">'
    for i in range(1, min(total_pages+1, 20)):  # cap pagination display
        href = "/blog/" if i == 1 else f"/blog/page/{i}/"
        if i == page_num: pag += f'<span class="cur">{i}</span>'
        else: pag += f'<a href="{href}">{i}</a>'
    if total_pages > 19: pag += f'<span>... {total_pages}</span>'
    pag += '</div>'
...
def page_post(p):
    ...
    exc = re.sub(r'<[^>]+>', '', p.get("excerpt","")).strip()[:200]
    schema = json.dumps({"@context":"https://schema.org","@type":"Article",
        "headline":p["title"],"author":{"@type":"Person","name":"Dr. Connor Robertson","url":SITE_URL},
        "datePublished":p["date"],"publisher":{"@type":"Person","name":"Dr. Connor Robertson"},"description":exc})
    extra = f'<script type="application/ld+json">{schema}</script>'
    return header(f'{p["title"]} | Dr. Connor Robertson', exc, p["relative_url"], extra) + f"""
<article class="post">
<h1>{p["title"]}</h1>
<p class="pm">{dt}</p>
<div class="pb">{p["content"]}</div>
...
def sitemap(posts):
    entries = ""
    for u in ["/","/about/","/speaker/","/books/","/press-media/","/contact/","/blog/"]:
        entries += f"  <url><loc>{SITE_URL}{u}</loc><changefreq>weekly</changefreq><priority>0.8</priority></url>\n"
    for p in posts:
        d = p["date"][:10]
...
def main():
    ...
    if no_fetch and CACHE_FILE.exists():
        print("\n[1/6] Loading cached posts...")
        with open(CACHE_FILE) as f:
            posts = json.load(f)
        print(f"  Loaded {len(posts)} posts from cache")
    else:
        print("\n[1/6] Fetching posts from WP API...")
    ...
    # Blog posts
    print(f"\n[4/6] Generating {len(posts)} blog post pages...")
    ...
    print("\n[5/6] Generating blog index pages...")
    per_page = 24
    ...
    # SEO files
    print("\n[6/6] Generating SEO and config files...")
